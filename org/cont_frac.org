...
Another example: the irrational number $e$ can be expressed as a continuous fraction by this code:
...
* Continued fraction approximation as recursive function compositions
...
Let's try it on the example given in the [[*Continued fraction approximation as recursive function compositions][previous section]]:
#+begin_src python :exports both :noweb strip-export :results output :dir ../src/
  <<DEMO_IMPORTS>>
...
The first row represents the terms of the continued fraction. To understand the other numbers, we calculate the matrices for each additional term in the continued fraction:
#+begin_src python :exports both :noweb strip-export :results output :dir ../src/
  <<DEMO_IMPORTS>>
...
The first step is to symbolically represent the function as a quotient and a remainder (see =qr()= in =r2cf_=). I implemented this operation in =qr_matrix= in the [[*Appendix: Quotients/remainders for matrices][Appendix]]. We can use it on the third convergent:
#+begin_src python :exports both :noweb strip-export :results output :dir ../src/
  <<DEMO_IMPORTS>>
...
So there is enough information in $\frac{70x+29}{12x+5}$ to precisely determine 3 terms (5, 1, 4). We stopped at the last term, because $\frac{2x+1}{2x}$ is an unbounded function. The quotient cannot be determined.
...
This is a just an extension of the format that we use to visualize =r2cf=: we start with $\begin{bmatrix} 70 & 29 \\ 12 & 5\end{bmatrix}$, end with $\begin{bmatrix}2 & 1 \\ 2 & 0\end{bmatrix}$, and output 5, 1, 4 in the process. The process stops when we don't have enough information to determine the next quotient. 
...
I need a utility function to manage the large number of numbers in the tensor:
...
Applying new terms is implemented with my favorite Numpy function: =einsum= (Einstein's sum; see [[https://ajcr.net/Basic-guide-to-einsum/][this tutorial]]):
#+begin_src python :noweb no :noweb-ref BIHOMOGRAPHY
...
* Convergents for continued fraction arithmetic
We have already developed sufficient machinery for doing continued fraction arithmetic. By updating the bihomographic function with new terms of $a$ and $b$, we can generate a series of rational numbers that converges to the result. The following code applies a new term of $a$, and then a new term of $b$... and so on. This is not the only way to do it. Any process that applies the terms in sequence will work. You can, for example, update with two terms of $a$, and then two terms of $b$.
#+begin_src python :tangle no :noweb-ref BIHOMOGRAPHY
...
We have calculated [[*Simple transformations of continued fractions][previously]] that $2*[2, 1, 1, 5, 1] = \frac{66}{13}$. Let's verify it by adding the continued fraction to itself:
#+begin_src python :exports both :noweb strip-export :results output :dir ../src/
  <<DEMO_IMPORTS>>
  a = iter([2, 1, 1, 5, 1])
  b = iter([2, 1, 1, 5, 1])
...
#+begin_src python :exports both :noweb strip-export :results output :dir ../src/
  ...
  a = iter([2, 1, 1, 5, 1])
  b = iter([2, 1, 1, 5, 1])
  print(arithmetic_convergents_tab(a, b, tForSubtraction))
#+end_src
...
#+begin_src python :exports both :noweb strip-export :results output :dir ../src/
  ...
  a = iter([2, 1, 1, 5, 1])
  b = iter([2, 1, 1, 5, 1])
  print(arithmetic_convergents_tab(a, b, tForDivision))
#+end_src
...
#+begin_src python :exports both :noweb strip-export :results output :dir ../src/
  ...
  a = islice(cf_sqrt2(), 6)
  b = islice(cf_sqrt2(), 6)  
  print(arithmetic_convergents_tab(a, b, tForMultiplication))
#+end_src
...
Unfortunately, these displays are not very satisfying, because the numbers tend to grow bigger and bigger. That's because we haven't performed any Euclidean step. That's for the next section.
...
#+begin_src python :tangle no :noweb-ref ARITHMETIC
  def cf_arithmetic_(cf_a: Iterator[int], cf_b: Iterator[int], t0: np.ndarray) -> Iterator:
      t = t0
  ...
  def cf_arithmetic(cf_a: Iterator[int], cf_b: Iterator[int], t0: np.ndarray) -> Iterator[int]:
