...
#+begin_src python :exports both :noweb strip-export :results output :dir ../src/
  <<DEMO_IMPORTS>>
  print(cf_transform_tab(iter([2, 1, 1, 5, 1, 3]), field_width = 3))
#+end_src
...
#+begin_src python :exports both :noweb strip-export :results output :dir ../src/
  ...
  print(cf_transform_tab(iter([2, 1, 1, 5, 1]), m0 = m0, field_width = 3))
#+end_src
...
#+begin_src python :exports both :noweb strip-export :results output :dir ../src/
  ...
  print(cf_transform_tab(cf_sqrt2(), m0 = m0, n = 6, field_width = 3))
#+end_src
...
#+begin_src python :exports both :noweb strip-export :results output :dir ../src/
  ...
  print(cf_transform_tab(cf_e(), m0 = m0, n = 12, field_width = 3))
#+end_src
...
* Arithmetic: bilinear homographic functions
When there is just one continued fraction $a$, the action is on the convergent of the form $f(x) = \frac{px+q}{rx+s}$, which can be represented with by a matrix $M = \begin{bmatrix} p & q \\ r & s \end{bmatrix}$. With two continued fractions $a$ and $b$, the function generalizes to $f(x, y) = \frac{axy + bx + cy + d}{exy + fx + gy + h}$, where $x$ and $y$ are the unknown terms for $a$ and $b$ respectively. This bilinear function can be represented by the 2x2x2 tensor $T=\begin{bmatrix}\begin{bmatrix} b & d \\ a & c \end{bmatrix}\begin{bmatrix} f & h \\ e & g \end{bmatrix}\end{bmatrix}$, where the first matrix encodes terms in the numerator, and the second encodes terms in the denominator. This particular representation is not necessary for the computation, but it gives the update rules a very nice visual interpretation. When the tensor is viewed as a 2x2x2 cube in 3D, it moves to leftwards when it receives a term of $a$, and it moves downwards when it receives a term of $b$. The following figure illustrates this for $a + b$. For each row, the table on the left illustrates the matrix encoding terms in the numerator, and the table on the right illustrates terms in the denominator. When the two matrices are combined, they describes the function in the middle panel. We start at the initial condition in the first row, add the first coefficient of $a$ in the second row, and then add the first coefficient of $b$ in the third row.

#+attr_html: :width 500px
[[../figs/addition.png]]

Given what we have learned in the previous section, it shouldn't be too surprising that multiplication can be performed with a different initial tensor (but it's still quite remarkable!)

#+attr_html: :width 500px
[[../figs/multiplication.png]]

Let's implement this with Numpy tensors:
