...
What if we want to calculate convergents as in =cf_convergent1=, but we also want them expressed as continued fractions? This seems to be a pointless exercise, because a continued fraction is what we start with! But it will become useful in the coming sections.
...
So what do we do? The answer is still the Euclidean algorithm, but have to use it symbolically rather than numerically.

The most important step in the Euclidean algorithm is to express a number in terms of the quotient and the reminder of another number (see =qr()= in =r2cf_=). I implemented the symbolic version of this operation in a function called =qr_matrix= in the Appendix. Consider the second convergent of $[5, 1, 4...]$, we use =qr_matrix= to calculate the quotient and the reminder:
...
#+begin_src python :exports both :noweb strip-export :results output :dir ../src/
  ...
  cf = islice(cf_transform(cf_sqrt2(), m0 = m0), 9)
  print(list(cf))
  cf_transform_tab(cf_sqrt2(), m0 = m0, n = 9, field_width = 3)
#+end_src
...
#+begin_src python :exports both :noweb strip-export :results output :dir ../src/
  ...
  cf = islice(cf_transform(cf_e(), m0 = m0), 2)
  #for i in cf:
  #    print(i)
  cf_transform_tab(cf_e(), m0 = m0, n = 2, field_width = 4)
#+end_src
...
* Appendix: Calculating the quotient and the reminder of a convergent in matrix form
